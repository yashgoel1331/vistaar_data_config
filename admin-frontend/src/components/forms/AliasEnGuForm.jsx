...
export default function AliasEnGuForm({ onPatch, onPut, onDelete, loading }) {
  ...
  return (
    <div className="space-y-3 border rounded p-4 bg-white">
      <h3 className="text-sm font-semibold text-gray-700">Alias en-gu actions</h3>
      <div>
        <label className="block text-xs text-gray-500 mb-1">canonical_en</label>
      ...
      <div>
        <label className="block text-xs text-gray-500 mb-1">gu_aliases (comma separated)</label>
        <input value={aliases} onChange={(e) => setAliases(e.target.value)}
          className="w-full border rounded px-3 py-2 text-sm" placeholder="દૂધ, દૂધમ" />
      </div>
